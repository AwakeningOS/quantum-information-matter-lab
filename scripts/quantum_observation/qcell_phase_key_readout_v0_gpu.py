#!/usr/bin/env python3
"""Delayed phase-key readout test for Q-cell internal structure."""
...
EXPERIMENT = "qcell_phase_key_readout_v0"
GRID_ID = "QFCBM_0988"
PREP_CYCLES = 120
READOUT_CYCLES = 80
SOURCE_ARMS = [
...
KEYS = ["no_key", "correct_key", "wrong_key", "phase_shuffled_correct_key"]
...
def apply_key(rho, key: str, seeds: list[int], grid, device):
    if key == "no_key":
        return rho
    cur = phase_shuffle(rho, seeds, device) if key == "phase_shuffled_correct_key" else rho
    if key in ("correct_key", "phase_shuffled_correct_key"):
        cur = eng.apply_two_local(cur, eng.exchange(-grid.g_internal, device), cpu.N_SYS, 3, 4)
        cur = apply_single(cur, rz(-math.pi / 2), 3, device)  # C phase reference.
        cur = apply_single(cur, ry(math.pi / 2), 4, device)   # D basis conversion before W.
    elif key == "wrong_key":
        cur = eng.apply_two_local(cur, eng.exchange(grid.g_internal, device), cpu.N_SYS, 3, 4)
        cur = apply_single(cur, rz(math.pi / 2), 3, device)
        cur = apply_single(cur, ry(-math.pi / 2), 4, device)
    else:
        raise ValueError(key)
    return cur


def readout_branch(grid, rho0, key: str, seeds: list[int], device):
    n = len(seeds)
    bits = torch.as_tensor(cpu.SYS_BITS, dtype=eng.RDTYPE, device=device)
    hamming = torch.as_tensor(cpu.HAMMING_SYS, dtype=eng.RDTYPE, device=device)
    zero = eng.t(cpu.r_state("R2_pure_0"), device)
    ...
    for _cycle in range(READOUT_CYCLES):
        keyed = apply_key(rho, key, seeds, grid, device)
...
def main() -> None:
    ...
    for source, variant, dep in SOURCE_ARMS:
        ...
        for key in KEYS:
            got = readout_branch(grid, rho, key, seeds, device)
            diag_got = readout_branch(grid, diagonal_shadow(rho), key, seeds, device)
    ...
    for source, _variant, _dep in SOURCE_ARMS:
        no = [r for r in rows if r["source_arm"] == source and r["key"] == "no_key"]
        no_by = {r["seed"]: r for r in no}
        for key in KEYS:
            sub = [r for r in rows if r["source_arm"] == source and r["key"] == key]
            gains = [r["readout_W"] - no_by[r["seed"]]["readout_W"] for r in sub]
            ...
            summary.append({
                ...
                "mean_key_gain_vs_no_key": sum(gains) / len(gains),
                "positive_key_gain_seed_count": sum(1 for g in gains if g > 0),
    ...
    manifest = {
        ...
        "claim_ceiling": "model-level delayed readout-key test; no quantum advantage or physical energy claim",
    }
    (out / f"{EXPERIMENT}_manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2), encoding="utf-8")
    lines = ["# Q-cell phase-key readout v0", "", "Date: 2026-07-10 JST", "", "## Summary", ""]
    for r in summary:
        lines.append(f"- {r['source_arm']} / {r['key']}: W `{r['mean_readout_W']:.6f}`, diag W `{r['mean_diag_shadow_readout_W']:.6f}`, coherence-attributable W `{r['mean_coherence_attributable_readout_W']:.6f}`, key gain `{r['mean_key_gain_vs_no_key']:.6f}`, positive coherence seeds `{r['positive_coherence_attributable_seed_count']}/{r['n_seed']}`, prep coherence `{r['mean_prep_coherence']:.6f}`, prep MI `{r['mean_prep_MI']:.6f}`.")
